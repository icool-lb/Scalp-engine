...
module.exports=async function(req,res){try{
 ...
 const marketDataHost=process.env.METAAPI_MARKET_DATA_HOST||`https://mt-market-data-client-api-v1.${region}.agiliumtrade.ai`;
 ...
 if(wantCandles){
   ...
   let url=`${marketDataHost}/users/current/accounts/${encodeURIComponent(accountId)}/historical-market-data/symbols/${encodeURIComponent(symbol)}/timeframes/${encodeURIComponent(tf)}/candles?limit=${limit}`;
   ...
   for(let i=0;i<4;i++){attempts=i+1;let up;try{up=await ft(url,{headers:{Accept:'application/json','auth-token':token}},10000)}catch(e){status=504;detail=e.message;if(i<3)await sleep(500*Math.pow(1.7,i));continue}
    ...
    if(up.ok){res.setHeader('Cache-Control','no-store,max-age=0');return res.status(200).json({ok:true,source:'metaapi-historical-candles',host:marketDataHost,symbol,timeframe:tf,attempts,candles:data})}
    detail=clean(data); if(![429,500,502,503,504].includes(status)||i===3)break; await sleep(500*Math.pow(1.7,i));
   }
   return res.status(status||500).json({ok:false,error:'MetaAPI candles failed',status,attempts,detail,hint: status===404?'Historical candles use a separate host: mt-market-data-client-api-v1.<region>.agiliumtrade.ai. If this still fails, verify account is deployed, symbol exists exactly as broker uses it, and try METAAPI_MARKET_DATA_HOST override from MetaApi API access page.':'Check MetaAPI account connection and market data access'});
 }
